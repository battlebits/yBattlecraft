...
public class CommandManager {
	...
	public CommandManager(Main m) {
		this.m = m;
		loadCommands(m, "br.com.battlebits.battlecraft.commands");
	}

	private void loadCommands(JavaPlugin plugin, String packageName) {
		for (Class<?> commandClass : ClassGetter.getClassesForPackage(m, packageName)) {
	...
	private void registerCommand(String name, CommandExecutor exc) throws Exception {
		PluginCommand command = m.getCommand(name.toLowerCase());
		if (command == null) {
			Constructor<?> constructor = PluginCommand.class.getDeclaredConstructor(String.class, Plugin.class);
			constructor.setAccessible(true);
			command = (PluginCommand) constructor.newInstance(name, m);
		}
		...
		m.getReflectionManager().getCommandMap().register(name, command);
	}
	...
	private void unregisterCommand(String name) {
		try {
			...
			Map<String, Command> knownCommands = (Map<String, Command>) known.get(m.getReflectionManager().getCommandMap());
			Set<String> aliases = (Set<String>) alias.get(m.getReflectionManager().getCommandMap());
