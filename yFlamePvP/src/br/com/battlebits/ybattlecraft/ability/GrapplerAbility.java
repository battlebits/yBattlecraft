			...
			if (e.getItem() != null) {
				if (e.getItem().equals(grapplerItem)) {
					...
					if (e.getAction() == Action.LEFT_CLICK_AIR || e.getAction() == Action.LEFT_CLICK_BLOCK) {
						...
						hook.spawn(e.getPlayer().getEyeLocation().add(e.getPlayer().getLocation().getDirection().getX(), e.getPlayer().getLocation().getDirection().getY(), e.getPlayer().getLocation().getDirection().getZ()));
						hook.move(e.getPlayer().getLocation().getDirection().getX() * 5.0D, e.getPlayer().getLocation().getDirection().getY() * 5.0D, e.getPlayer().getLocation().getDirection().getZ() * 5.0D);
						...
						if (grapplerHooks.containsKey(e.getPlayer().getUniqueId())) {
							if (grapplerHooks.get(e.getPlayer().getUniqueId()).isHooked()) {
								...
								double t = d;
								double v_x = (1.0D + 0.04000000000000001D * t) * ((isNear(loc, e.getPlayer().getLocation()) ? 0 : loc.getX() - e.getPlayer().getLocation().getX()) / t);
								double v_y = (0.9D + 0.03D * t) * ((isNear(loc, e.getPlayer().getLocation()) ? 0.1 : loc.getY() - e.getPlayer().getLocation().getY()) / t);
								double v_z = (1.0D + 0.04000000000000001D * t) * ((isNear(loc, e.getPlayer().getLocation()) ? 0 : loc.getZ() - e.getPlayer().getLocation().getZ()) / t);
								Vector v = e.getPlayer().getVelocity();
								v.setX(v_x);
								v.setY(v_y);
								v.setZ(v_z);
