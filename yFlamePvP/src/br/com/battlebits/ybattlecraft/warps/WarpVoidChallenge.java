...
public class WarpVoidChallenge extends BaseWarp {
	...
	public void onTeleport(PlayerWarpJoinEvent event) {
		if (isOnWarp(event.getPlayer())) {
			...
			if (time.containsKey(p.getUniqueId())) {
				p.sendMessage("§5§lVOID CHALLENGE §fVoc§ sobreviveu por §9§l" + Battlecraft.getTimeUtils()
		...
		if (e.getCause() == DamageCause.FALL) {
			if (e.getEntity() instanceof Player) {
				...
				if (isOnWarp(p)) {
					...
					p.sendMessage("§5§lVOID CHALLENGE §fVoc§ deve pular no §9§lVOID §fpara iniciar o desafio.");
				}
			}
		}
	}
